...
def language_eval(sample_seqs, groundtruth_seqs):
    ...
    for i in range(len(groundtruth_seqs)):
        refs = [sanitize_string(groundtruth_seqs[i][j]) for j in range(len(groundtruth_seqs[i])) if groundtruth_seqs[i][j].strip()]
        if not refs:
            refs = ["dummy reference"]
        references[i] = refs
    for i in range(len(sample_seqs)):
        pred = sanitize_string(sample_seqs[i])
    ...
    #     print(f"Sample {i}:")
    #     print(f"  Predicted: {predictions[i][0]}")
    #     print(f"  References: {references[i][:3]}")  # 限制显示前3条参考描述
    #     print()
