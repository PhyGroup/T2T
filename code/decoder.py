...
class Decoder(nn.Module):
    def __init__(self, output_dim, emb_dim, hidden_dim, num_layers, dropout, attention, bert_embedding_matrix=None, action_embedding_dim=None):
        super().__init__()
        self.output_dim = output_dim
        self.attention = attention
        self.embedding = nn.Embedding.from_pretrained(bert_embedding_matrix, freeze=False) if bert_embedding_matrix is not None else nn.Embedding(output_dim, emb_dim)
        self.rnn = nn.LSTM(emb_dim + hidden_dim + action_embedding_dim, hidden_dim, num_layers, dropout=dropout if num_layers > 1 else 0, batch_first=True)
