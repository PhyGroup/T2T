...
class TrafficCaptionDataset(Dataset):
    def __init__(self, csv_path, descriptions_dict, sample_ids, vocab, app_type_map, sbert_embeddings_path, max_traffic_seq_len=50):
            ...
            try:
                ...
                if current_dim != EMBEDDING_DIM_TRAFFIC:
                    if current_dim > EMBEDDING_DIM_TRAFFIC:
                        ...
                        padding = pd.DataFrame(0, index=numeric_df.index, columns=[f'pad_{i}' for i in range(EMBEDDING_DIM_TRAFFIC - current_dim)])
                ...
                valid_descs = [d for d in descs if isinstance(d, str) and d.strip() and len(self.vocab.preprocess_sentence(d)) >= 2]
                if not valid_descs:
                    continue
                ...
                if sample_caption_embeddings is None or sample_caption_embeddings.shape[0] != len(valid_descs):
                    print(f"Warning: Mismatch in SBERT embeddings for {sample_id}. Expected {len(valid_descs)} but got {sample_caption_embeddings.shape[0] if sample_caption_embeddings is not None else 'None'}. Skipping.")
                    continue
                for desc, caption_emb in zip(valid_descs, sample_caption_embeddings):
                    data_pairs.append((traffic_features, desc, sample_id, self.app_type_map[sample_id], caption_emb.to(DEVICE)))
            except Exception as e:
    ...
    def __getitem__(self, index):
        ...
        numericalized_caption = [self.vocab.stoi["<sos>"]] + self.vocab.numericalize(description) + [self.vocab.stoi["<eos>"]]
