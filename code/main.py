...
from config import *
from dataset import Vocabulary, TrafficCaptionDataset, collate_fn, load_bert_embeddings_for_vocab
from encoder import TransformerEncoder, Seq2Seq
from decoder import Attention, Decoder
from train import train_epoch
from eval import evaluate_model
from infer import predict_app_type, generate_caption
...
if __name__ == "__main__":
    ...
    for s_id in video_descriptions:
        ...
        has_valid_descriptions = any(isinstance(d, str) and d.strip() and len(Vocabulary.preprocess_sentence(d)) >= 2 for d in descs)
        if match and general_app_type_str in app_type_to_id and has_valid_descriptions:
            filtered_video_descriptions[s_id] = [d for d in descs if isinstance(d, str) and d.strip() and len(Vocabulary.preprocess_sentence(d)) >= 2]
    ...
    valid_sample_ids = [s_id for s_id in filtered_video_descriptions if (CSV_FEATURE_PATH / f"{s_id}.csv").exists() and s_id in sample_id_to_app_type]
    ...
    bert_embedding_matrix = load_bert_embeddings_for_vocab(vocab, BERT_MODEL_PATH, DEVICE) if Path(BERT_MODEL_PATH).exists() else None
    TEXT_EMBEDDING_DIM = bert_embedding_matrix.shape[1] if bert_embedding_matrix is not None else EMBEDDING_DIM_TEXT

    train_dataset = TrafficCaptionDataset(CSV_FEATURE_PATH, filtered_video_descriptions, train_ids, vocab, sample_id_to_app_type, SBERT_EMBEDDINGS_PATH, MAX_TRAFFIC_SEQ_LEN)
    val_dataset = TrafficCaptionDataset(CSV_FEATURE_PATH, filtered_video_descriptions, val_ids, vocab, sample_id_to_app_type, SBERT_EMBEDDINGS_PATH, MAX_TRAFFIC_SEQ_LEN)
    test_dataset = TrafficCaptionDataset(CSV_FEATURE_PATH, filtered_video_descriptions, test_ids, vocab, sample_id_to_app_type, SBERT_EMBEDDINGS_PATH, MAX_TRAFFIC_SEQ_LEN)

    actual_train_batch_size = min(BATCH_SIZE, len(train_dataset)) if len(train_dataset) > 0 else 1
    train_loader = DataLoader(train_dataset, batch_size=actual_train_batch_size, shuffle=True, collate_fn=collate_fn, drop_last=(len(train_dataset) >= BATCH_SIZE))
    ...
    model = Seq2Seq(encoder, decoder, DEVICE, NUM_APP_TYPES, ACTION_EMBEDDING_DIM, NUM_ACTION_PROTOTYPES, TEXT_EMBEDDING_DIM).to(DEVICE)
    ...
    if len(train_dataset) > 0:
        for epoch in range(NUM_EPOCHS):
            train_caption_loss, train_app_loss, train_distance_loss, train_contrastive_loss, train_total_loss = train_epoch(
                model, train_loader, optimizer, DEVICE, TEACHER_FORCING_RATIO)
            val_scores = evaluate_model(model, val_loader, vocab, DEVICE, coco_scorers, APP_TYPES)
            print(f"\nEpoch {epoch + 1}/{NUM_EPOCHS} | Train Caption Loss: {train_caption_loss:.4f} | Train App Loss: {train_app_loss:.4f} | Train Distance Loss: {train_distance_loss:.4f} | Train Contrastive Loss: {train_contrastive_loss:.4f} | Train Total Loss: {train_total_loss:.4f}")
    ...
    if len(test_ids) > 0:
        ...
        for sample_id in sample_ids_to_show:
            ...
            if found_sample:
                ...
                with torch.no_grad():
                    app_type_pred = predict_app_type(model, tf.unsqueeze(0).to(DEVICE))
                    predicted_app_id = app_type_pred.argmax(1).item()
                    generated_text, _ = generate_caption(model, tf.unsqueeze(0).to(DEVICE), vocab, predicted_app_id)
